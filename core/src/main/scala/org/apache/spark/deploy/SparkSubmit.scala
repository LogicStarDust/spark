...
object SparkSubmit {
  ...
  def main(args: Array[String]): Unit = {
    ...
    appArgs.action match {
      // 根据参数解析后的action，决定对于一个应用提交、杀死或请求状态。后面两个操作只支持standalone cluster模式。
  ...
  /**
    ...
    *
    * 这个运行主要分为两步。
    * 第一步，我们准备启动环境通过设置合适的classpath、system properties、在cluster manager上运行的主类的应用参数、部署模式。
    * 第二部，我们使用启动环境调用主类的main方法
    *
  ...
  private def submit(args: SparkSubmitArguments): Unit = {
    ...
    /**
      * 在standalone cluster 模式，有两种提交通道：
      * (1)传统的Akka通道使用o.a.s.deploy.Client为一个封装
      * (2)新的REST-based通道在Spark 1.3引入
      * 后者是默认的操作作为Spark 1.3，但是spark submit将会失败
      * 在Master端不是一个REST Server时候使用遗留的通道
      * ——
      * 否则，在所有其他的模式，只要运行主类就可以
      *
      */
